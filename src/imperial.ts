...
export class ImperialNotation extends Notation {
  ...
  public formatDecimal(value: Decimal): string {
    ...
    let logValue = new Decimal(10).pow(value.log10().min(LOG_MAX_VOLUME)).exponent + + Math.log(Math.max(new Decimal(10).pow(value.log10().min(LOG_MAX_VOLUME)).mantissa, 1));
